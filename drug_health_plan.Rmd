...
ggplot(data = plotdata, aes(factor(Star_Rating_Current), average_physician)) +
    ...
    guides(fill=FALSE) +
    labs(x="Rating Scores", y="Average Physician Number") +
    geom_boxplot(aes(fill = factor(Star_Rating_Current)))
...
```{r,cache=TRUE,warning=FALSE}
# train the model
logsitic_regression_model <- glm(formula = sentiment ~ -sentiment, family = binomial(link = 'logit'), data = train)
