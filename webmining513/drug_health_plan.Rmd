...
<!-- #tail(med_data, 10) -->
<!-- ``` -->
#Insurance Coverage by CountyFPISCode
```{r,cache=TRUE}
...
```
#Hospital Type by Zipcode, County Name, or State
```{r,cache=TRUE}
...
unemployment_data2$unemployment_rate <- as.numeric(unemployment_data2$unemployment_rate) / 100
...
<!-- head(income_data2) -->
<!-- ``` -->
# Combine data by zip code or county
```{r,cache=TRUE}
mdata <- merge(hospital_data2, unemployment_data2, all = TRUE, by.x = "ZIP.Code", by.y = "zipcode")
mdata2 <- na.omit(mdata)
head(mdata2)
```
