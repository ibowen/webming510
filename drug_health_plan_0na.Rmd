...
insurance_data$CountyFPISCode <- with(insurance_data, paste(state, county, sep = ""))
insurance_data$PCTIC_PT <- as.numeric(insurance_data$PCTIC_PT)
#write.csv(json_data1, file = "./insurance_coverage_2013.csv")
insurance_data2 <- insurance_data[, c("PCTIC_PT", "CountyFPISCode")]
str(insurance_data2)
...
colnames <- c('PCTIC_PT', 'experience', 'spec_count', 'unemployment_rate', 'averge_income', 'avgerage_physician', 'tier_label', 'mnthly_prm', 'ann_ddctbl', 'copay_max', 'coin_max', 'Star_Rating_Current')
final_data2 <- final_data[, colnames]
str(final_data2)
tail(final_data2)

write.csv(final_data, file = "./data/final_data_v2.csv")
# unemploy_Income_0na
# cost_rating_zip2
```
# Logistic Regression
...
final_data$avgerage_physician = round(final_data$polulation / (final_data$count + 1))
# remove neutral 4 points, and change the label into 1 and 0
final_data_rm4 <- final_data[final_data$Star_Rating_Current != 4, ]
# transfer the label into binary value
final_data_rm4$sentiment <- as.numeric(ifelse(final_data_rm4$Star_Rating_Current > 4,1,0))
...
final_data_rm4 <- final_data_rm4[, colnames]
# subset the train and test data sets
set.seed(510)
sample_size <- floor(0.8 * nrow(final_data_rm4))
training_index <- sample(seq_len(nrow(final_data_rm4)), size = sample_size)
train <- final_data_rm4[training_index,]
test <- final_data_rm4[-training_index,]
# train the model
model <- glm(formula = sentiment ~ ., family = binomial(link = 'logit'), data = train, maxit=1000)
summary(model)
# evaluate the model
...
# Logistic Regression including insurance coverage
# remove neutral 4 points, and change the label into 1 and 0
...
# subset the need columns for modeling
colnames <- c('PCTIC_PT', 'experience', 'spec_count', 'unemployment_rate', 'averge_income', 'avgerage_physician', 'tier_label', 'mnthly_prm', 'ann_ddctbl', 'copay_max', 'coin_max', 'sentiment')
final_data2_rm4 <- final_data2_rm4[, colnames]
str(final_data2_rm4)
# subset the train and test data sets
set.seed(510)
sample_size <- floor(0.6 * nrow(final_data2_rm4))
